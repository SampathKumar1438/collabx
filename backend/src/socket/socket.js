...
export function setupSocketHandlers(io) {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            // prisma.user.findUnique uses 'userId' as the primary key field name based on schema
            ...
            // You can add an isActive check here if your schema supports it, 
            // currently schema doesn't show 'isActive' explicitly but let's assume valid user is enough
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('call:start', async ({ recipientId, chatId, offer, isVideo, isGroup }) => {
            ...
            if (isGroup && chatId) {
                ...
                // 1-to-1 can also use the "room" approach for consistency, or keep direct messaging.
                // Keeping direct for now to allow using the 'offer' passed in start (optimization).
                // BUT, to unify UI, we might want to switch to room-based soon.
                // For now, let's keep the legacy 1-to-1 flow if 'isGroup' is false.
        ...
        socket.on('call:join', ({ chatId }) => {
            ...
            // Also notify ME about who is already there? 
            // Actually, if they send offers, I will know them.
            // But usually best to ask "who is there?" or just let them announce.
        });
