...
function Inbox({ chatData, onBack }) {
  ...
  const [onlineStatus, setOnlineStatus] = useState(chatData?.isOnline || false);
  const [lastActiveAt, setLastActiveAt] = useState(
    chatData?.lastActiveAt || null
  );
  ...
  useEffect(() => {
    if (conversationId) {
      setPage(0);
      setHasMore(true);
      setSearchQuery(""); // Reset search on chat change
      setIsSearching(false);
      fetchMessages(0, true); // Reset to first page
      conversationsAPI.markAsRead(conversationId);
      const savedBg = localStorage.getItem(`chat_bg_dm_${conversationId}`);
      if (savedBg) {
        try {
  ...
  useEffect(() => {
    setOnlineStatus(chatData?.isOnline || false);
    setLastActiveAt(chatData?.lastActiveAt || null);
  }, [chatData]);
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (messageData) => {
      ...
      // Mark as read immediately when receiving a message in active chat
      conversationsAPI.markAsRead(conversationId);
      ...
      const newMsg = {
        ...
        sender: messageData.sender?.username || "User",
        avatar: messageData.sender?.profilePictureUrl || null,
      ...
      console.log("DEBUG: New Msg received", {
        msgId: newMsg.id,
      ...
      setMessages((prev) => {
        // Prevent duplicate messages (in case we already added it via API response)
    ...
    const handleUserTyping = ({ chatId, username }) => {
      console.log("TYPING EVENT RECEIVED:", { chatId, username });
    ...
    const handleUserPresence = ({ userId, isOnline: online }) => {
  ...
  }, [socket, conversationId, chatData]);
    ...
    try {
      ...
      if (response.success) {
        ...
        const transformedMessages = response.data.map((msg) => {
          ...
          // Debug alignment on first load
          if (pageNum === 0 && Math.random() < 0.1) {
            console.log("DEBUG MSG ALIGNMENT:", {
              msgId: msg.messageId,
    ...
    if (conversationId) {
      bg
        ? localStorage.setItem(
            `chat_bg_dm_${conversationId}`,
            JSON.stringify(bg)
          )
        : localStorage.removeItem(`chat_bg_dm_${conversationId}`);
    }
  };

  const handleSend = async ({ text, files, audio, gifUrl }) => {
    if (!conversationId) return;
  ...
  // Get user data from chatData
  const user = chatData
    ? {
        id: chatData.id || chatData.conversation?.conversationId,
        name: chatData.name || "Unknown User",
        avatar: chatData.avatar || null,
        status: onlineStatus ? "Active now" : "Offline",
    ...
    <div className="flex h-full w-full gradient-bg overflow-hidden transition-all duration-300 backdrop-blur-md">
      ...
      <div className="flex-1 flex flex-col h-full border-l border-stroke/20 dark:border-strokedark/20 overflow-hidden">
        <ChatHeader
          ...
          onProfileClick={() => setInfoOpen((prev) => !prev)}
        ...
        {uploadProgress > 0 && uploadProgress < 100 && (
          <div className="px-6 py-3 border-t border-stroke dark:border-strokedark gradient-bg-subtle">
            <div className="flex items-center gap-3">
              <div className="flex-1">
                ...
                <div className="w-full bg-gray-200 dark:bg-boxdark rounded-full h-2">
                  <div
      ...
      {/* Info Sidebar (Right) */}
      {infoOpen && (
        <div className="fixed md:relative inset-y-0 right-0 w-full md:w-[320px] h-full border-l border-stroke/20 dark:border-strokedark/20 gradient-bg-sidebar shadow-xl z-50 md:z-20 backdrop-blur-md">
          <ChatInfo
            chatData={user}
